...
const NewOwner = ({navigation, route}) => {
  const owner = route.params?.owner ? route.params.owner : null;

  const [formData, setFormData] = useState(
    owner ? getObject(owner) : defaultValueForm,
  );

  const [loading, setLoading] = useState(false);
  const [documentType, setDocumentType] = useState(
    owner ? owner.document_type : 'Cédula',
  );
  const [showPassword, setShowPassword] = useState(false);
  const [showConfirmPassword, setShowConfirmPassword] = useState(false);

  const ownerContext = useContext(OwnersContext);
  const {errors, created, createOwner, error, message, clearErrors} =
    ownerContext;

  //detectar cuando se esta editando o no
  useEffect(() => {
    if (created) {
      navigation.navigate('owners-index');
    }
  }, [created]);
